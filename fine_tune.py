...
def fine_train():
    ...
    de_dataset = classification_dataset(train_path, config.image_size,
                                        config.per_batch_size, 1,
                                        config.rank, config.group_size, num_parallel_workers=4)

    config.steps_per_epoch = de_dataset.get_dataset_size()

    # eval dataloader
    test_path = os.path.join(config.data_path, 'test')
    val_dataset = classification_dataset(test_path, image_size=config.image_size,
